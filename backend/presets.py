"""Preset helper functions extracted from :mod:`core`."""
...
def load_workout_presets(db_path: Path = DEFAULT_DB_PATH):
    """Load workout presets from the database into ``core.WORKOUT_PRESETS``."""
    ...
    core.WORKOUT_PRESETS = presets
    return presets


def find_presets_using_exercise(
    exercise_name: str, *, db_path: Path = DEFAULT_DB_PATH
) -> list[str]:
