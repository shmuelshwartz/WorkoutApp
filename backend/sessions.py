"""Helpers for validating and saving workout sessions."""
...
from .utils import _to_db_timing
...
def save_completed_session(
    ...
    with sqlite3.connect(str(path)) as conn:
        ...
        cursor.execute(
            """
            INSERT INTO session_preset_sections (session_id, preset_name)
            VALUES (?, ?)
            """,
        ...
        for ex_pos, ex in enumerate(session.exercises, 1):
            ...
            for set_idx, result in enumerate(ex.get("results", []), 1):
                ...
                for name, value in metrics.items():
                    ...
                    cursor.execute(
                        """
                        INSERT INTO session_set_metrics (exercise_metric_id, exercise_set_id, value)
                        VALUES (?, ?, ?)
                        """,
                        (metric_id, set_id, str(value)),
                    )
