...
def create_indexes(src_cur: sqlite3.Cursor, dst_cur: sqlite3.Cursor) -> None:
    ...
    dst_cur.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS idx_library_exercises_name_unique
        ON library_exercises(name);
        """
    )
    dst_cur.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS idx_library_metric_types_name_unique
...
def migrate(src: str, dst: str) -> None:
    ...
    try:
        ...
        try:
            dst_con.close()  # type: ignore[has-type]
        except Exception:
            pass
        if os.path.exists(dst):
            os.remove(dst)
