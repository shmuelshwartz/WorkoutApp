"""Full-screen metric selection dialog.

This screen was migrated from ``ui/popups.py`` where it was implemented as a
popup. It now uses :class:`MDScreen` to provide full-screen behaviour while
maintaining the original layout and callbacks.
"""
...
# Order of fields for metric editing widgets
METRIC_FIELD_ORDER = [
    "name",
    "description",
    "type",
...
class AddMetricPopup(MDScreen):
    """Screen-based dialog for selecting or creating metrics."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.name:
            self.name = f"_add_metric_{id(self)}"

        if self.mode == "session":
            content = MDBoxLayout(size_hint=(1, 1))
            close_btn = MDRaisedButton(text="Close", on_release=self.close)
            self._build_layout("Metrics Locked", content, [close_btn])
            return
        ...
        self._build_layout(title, content, buttons)
    ...
    def close(self, *_) -> None:
        """Return to the previous screen and remove this dialog."""
        app = MDApp.get_running_app()
    ...
    def _build_new_metric_widgets(self):
        ...
        back_btn = MDRaisedButton(text="Back", on_release=self.close)
        return scroll, [save_btn, back_btn], "New Metric"
    ...
    def _build_choice_widgets(self):
        ...
        return content, [add_btn, new_btn, cancel_btn], "Metric Options"
    ...
    def show_new_metric_form(self, *args):
        ...
        popup = AddMetricPopup(
            self.screen, self.previous_screen, popup_mode="new", mode=self.mode
        )
        popup.open()
    ...
    def show_metric_list(self, *args):
        self.close()
        popup = AddMetricPopup(
            self.screen, self.previous_screen, popup_mode="select", mode=self.mode
        )
        popup.open()
