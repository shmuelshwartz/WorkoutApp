"""Full-screen dialog for editing an existing metric.

Migrated from :mod:`ui.popups` where it was implemented as a popup. It now
subclasses :class:`~kivymd.uix.screen.MDScreen` to behave as an overlay screen
that mimics popup behaviour.
"""
...
from ui.popups import METRIC_FIELD_ORDER


class EditMetricPopup(MDScreen):
    """Screen-based dialog for editing an existing metric."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.mode == "session":
            content = MDBoxLayout()
            close_btn = MDRaisedButton(text="Close", on_release=lambda *a: self.close())
    ...
    # Layout helpers
    # ------------------------------------------------------------------
    def _build_layout(self, title: str, content, buttons):
        """Assemble the header, content and buttons into the screen."""
    ...
    def _build_widgets(self):
        ...
        cancel_btn = MDRaisedButton(text="Cancel", on_release=lambda *a: self.close())
    ...
    def close(self, *args):
        """Return to the screen that was active before opening."""
        if self.manager:
            self.manager.current = self.previous_screen
            self.manager.remove_widget(self)
        app = MDApp.get_running_app()
        if hasattr(app, "_dialog_stack") and app._dialog_stack:
    ...
    def save_metric(self, *args):
        ...
        if from_preset:
            ...
            rows = []
            if metrics.is_metric_type_user_created(self.metric["name"], db_path=db_path):
                ...
                row.add_widget(
                    MDLabel(
                        text="Set as default metric type",
                        halign="left",
                    )
                )
                rows.append(row)
